...
class Muse(FMRIWordLevel):
    ...
    def set_muse_param(self, layer, sub, bin_name, fold=None):
        ...
        self.config.muse_parameters.verbose = 0
    ...
    def run(self, extend_exp=None):
        ...
        project_name = f"{self.dataset_name}-{self.fmri_type}-brain2{self.model_type}_{project_type}_{self.suffix}{exp_flag}-get-best"
        wandb.init(project=project_name, name=f"{self.src_lang}_{self.tgt_lang}",
                   tags=[f"{self.src_lang}", f"{self.tgt_lang}"])
        metrics_df = pd.DataFrame()

        for layer in range(37,38):
...
def muse_supervised(configs):
    ...
    # Save the best project (optional)
    exp_path = Path(f"./{params.tgt_lang}_best")
    exp_path.mkdir(exist_ok=True,parents=True)
    params.exp_path = str(exp_path)
