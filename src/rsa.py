...
class RSA(Muse):
        ...
        for layer in range(self.layers):
            ...
            for sub in range(1, self.subjects + 1):
                brain_data = torch.load(
                    self.outfile_dir
                    / f"{self.dataset_name}-{self.fmri_type}-sub--{sub}-{self.lookback}-{self.lookout}-{self.vec_dim}.pth"
                )
                ...
                lm_lst = [
                    lm_words.index(w.lower())
                    if self.if_cased == "uncased"
                    else lm_words.index(w)
                ...
                project_name = (
                    f"{self.dataset_name}-brain2{self.model_alias}_rsa_{self.suffix}"
                )
                wandb_tags = [f"sub_{sub}", f"{self.model_name}", f"layer_{layer}"]

                wandb.init(
                    project=project_name, name="_".join(wandb_tags), tags=wandb_tags
                )
                rsa_results.append((sub, layer, rho, p))
                wandb.log({"rsascore": rho, "pvalue": p})
                wandb.finish()
        return rsa_results
    ...
    def run_rsa_debug(self, extend_exp=None):
        ...
        project_name = (
            f"{self.dataset_name}-rsa-brain2{self.model_type}_{self.suffix}{exp_flag}"
        )
        wandb.init(
            project=project_name,
            name=f"{self.src_lang}_{self.tgt_lang}",
        ...
        for layer in range(self.layers):
            ...
            for sub in range(1, self.subjects + 1):
                brain_data = torch.load(
                    self.outfile_dir
                    / f"{self.dataset_name}-{self.fmri_type}-sub--{sub}-{self.lookback}-{self.lookout}-{self.tr_num}.pth"
                )

                metrics = {
                    "Subjects": f"subject-{sub}",
