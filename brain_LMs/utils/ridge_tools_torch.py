...
def kernel_ridge_torch(X_torch, Y_torch, lmbda):
    ...
    coeffs = torch.inverse(
        X_torch @ X_torch.transpose(0, 1)
        + lmbda * torch.eye(X_torch.shape[0], device=X_torch.device)
    )
...
def kernel_ridge_by_lambda_torch(
...
def cross_val_ridge(
    ...
    ridge_1 = dict(
        plain=ridge_by_lambda_torch, kernel_ridge=kernel_ridge_by_lambda_torch
    )[method]
    ridge_2 = dict(plain=ridge_torch, kernel_ridge=kernel_ridge_torch)[method]
