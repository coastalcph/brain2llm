...
def get_nlp_features_fixed_length(layer, seq_len, feat_type, feat_dir, train_indicator, SKIP_WORDS=20, END_WORDS=5176):
    ...
    pca = PCA(n_components=10, svd_solver='full') # 501 is over 0.95 for base
    pca.fit(train)
...
def prepare_fmri_features(predict_feat_dict,train_features, test_features, word_train_indicator, TR_train_indicator, SKIP_WORDS=20, END_WORDS=5176):
    ...
    # pca.fit(tmp[TR_train_indicator])
    # train_features_p = pca.transform(tmp[TR_train_indicator])
    # test_features_p = pca.transform(tmp[~TR_train_indicator])
    #
    # return tmp[TR_train_indicator], tmp[~TR_train_indicator], train_features_p, test_features_p
...
def run_class_time_CV_fmri_crossval_ridge(data, predict_feat_dict,
    ...
    for ind_num in range(n_folds):
        ...
        tmp_train_features, tmp_test_features, tmp_train_features_pca, tmp_test_features_pca = get_nlp_features_fixed_length(
            layer, seq_len, nlp_feat_type, feat_dir, word_CV_ind)
        train_features, test_features= prepare_fmri_features(predict_feat_dict, tmp_train_features, tmp_test_features,
        ...
        pca = PCA(n_components=512, svd_solver='full') # 437 is over 0.95
        pca.fit(train_data)
        ...
        # corrs[ind_num,:] = corr(preds,test_features)
