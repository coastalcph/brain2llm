...
class LMEmbedding:
    ...
    def get_lm_layer_representations(self):
        cache_path = (
            Path.home() / ".cache/huggingface/transformers/models" / self.model_name
        )
        tokenizer = AutoTokenizer.from_pretrained(
            self.pretrained_model, cache_dir=cache_path, use_fast=False
        )
        max_memory = {k: "40GB" for k in self.device}
        if self.model_alias == "bert":
            model = AutoModel.from_pretrained(
                self.pretrained_model, cache_dir=cache_path, output_hidden_states=True
            )
        ...
        for sent_idx, sentences in enumerate(self.text_sentences_array):
            ...
            sentences_words = [
                FMRIWordLevel.remove_punct(w)[0] for w in sentences.strip().split(" ")
            ]
            ...
            lm_dict = self.add_token_embedding_for_specific_word(
                ...
                sentences_words,
                lm_dict,
                is_avg=self.is_average,
            )

            if sent_idx % 100 == 0:
                print(
                    f"Completed {sent_idx} out of {len(self.text_sentences_array)}: {tm.time() - start_time}"
                )
                start_time = tm.time()
    ...
    def get_word_ind_to_token_ind(
        self, words_in_array, sentence_words, tokenizer, words_mask
    ):
        ...
        for i, word in enumerate(sentence_words):
            word_ind_to_token_ind[
                i
            ] = []  # initialize token indices array for current word
    ...
    def add_word_lm_embedding(
        lm_dict, embeddings_to_add, token_inds_to_avrg, specific_layer=-1
    ):
        if specific_layer >= 0:  # only add embeddings for one specified layer
            ...
            lm_dict[specific_layer].append(
                np.mean(full_sequence_embedding[0, token_inds_to_avrg, :], 0)
            )
        else:
    ...
    def add_token_embedding_for_specific_word(
        self, word_seq, tokenizer, model, sentence_words, lm_dict, is_avg=True
    ):
        all_sequence_embeddings, lm_dict = self.predict_lm_embeddings(
            word_seq, tokenizer, model, lm_dict
        )
        word_ind_to_token_ind = self.get_word_ind_to_token_ind(
            word_seq, sentence_words, tokenizer, list(word_seq)
        )
        ...
        for token_inds_to_avrg in list(word_ind_to_token_ind.keys()):
            ...
            lm_dict = self.add_word_lm_embedding(
                lm_dict, all_sequence_embeddings, token_ind
            )
