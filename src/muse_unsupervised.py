...
class Muse(FMRIWordLevel):
    ...
    def set_muse_param(self, layer, sub, bin_name, fold=None):
        ...
        self.config.muse_parameters.verbose = 2
        ...
        if self.model_alias == "ft" or self.model_alias == "openai" or self.model_alias == "bg":
            muse_params.tgt_emb = f"{self.word_decon_embs_dir}/{self.model_name}/{self.dataset_name}_{self.model_name}_dim_{self.vec_dim}_layer_{layer}.pth"
        else:
