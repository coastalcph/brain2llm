...
from src.fmri_dataloader_debug import FMRIWordLevel
...
class Muse(FMRIWordLevel):
    def __init__(self, config, train_eval, fmri_type="type"):
        ...
        self.do = train_eval
        self.vec_dim = config.model.dim
        self.src_lang = config.muse_parameters.src_lang
        self.tgt_lang = config.muse_parameters.tgt_lang

    def set_muse_param(self, muse_params, layer, sub, bin_name, fold=None):
        muse_params["src_lang"] = f"subject-{sub}"
        muse_params["tgt_lang"] = f"{self.model_name}.layer_{layer}"
        muse_params["verbose"] = 2 if layer == 0 and sub == 1 else 0
        # muse_params["cuda"] = False
    ...
    def run(self, extend_exp=None):
        ...
        muse_params = self.config.muse_parameters.to_dict()
                ...
                for bin_name in bins:
                    ...
                    if self.do == "train":
                        for fold in range(self.num_folds):
                            ...
                            muse_params = self.set_muse_param(muse_params, layer, sub, bin_name, fold)
                        ...
                        muse_params = self.set_muse_param(muse_params, layer, sub, bin_name)
    ...
    # def run_debug(self, extend_exp=None):
    #     exp_flag = "" if extend_exp is None else f"_{extend_exp}"
    #     bins = {
    #         "_freq": ["_freq500", "_freq5000", "_freq_end"],
    #         "_poly": ["_in_babelnet", "_out_babelnet"],
    ...
    #
    #     metrics_df = pd.DataFrame()
    #     muse_params = copy.deepcopy(self.config.muse_parameters).to_dict()
    #
    ...
    #                     if self.do == "train":
    #                         for fold in range(self.num_folds):
    #                             # Use apply_async to submit tasks to the pool
    #                             muse_params = self.set_muse_param(muse_params,layer, sub, bin_name, fold)
    #                             result = pool.apply_async(process_task, args=(layer, sub, bin_name, extend_exp, self.model_name, self.do, muse_params, fold))
    #                             # Get the result from the task and append to metrics_df
    #                             # results.append(result.get())
    #                             metrics_df = pd.concat([metrics_df, pd.DataFrame(result.get(), index=[0])])
    #                     else:
    #                         muse_params = self.set_muse_param(muse_params,layer, sub, bin_name)
    #                         result = pool.apply_async(process_task, args=(layer, sub, bin_name, extend_exp, self.model_name, self.do, muse_params))
    #                         # results.append(result.get())
    #                         metrics_df = pd.concat([metrics_df, pd.DataFrame(result.get(), index=[0])])
    #
    ...
    #     # pool.close()
    #     # pool.join()
    #     # metrics_df = pd.concat([pd.DataFrame(result, index=[0]) for result in results])
    #     wandb.log({"Results": wandb.Table(dataframe=metrics_df.round(2))}, commit=True)
    #     wandb.finish()
    ...
    #                             # Use apply_async to submit tasks to the pool
    #                             muse_params = self.set_muse_param(muse_params,layer, sub, bin_name, fold)
    #                             task = pool.apply_async(process_task, args=(layer, sub, bin_name, extend_exp, self.model_name, self.do, muse_params, fold))
    #                             # Get the result from the task and append to metrics_df
    #                             tasks.append(task)
    #                             metrics_df = pd.concat([metrics_df, pd.DataFrame(task.get(), index=[0])])
    #                     else:
    #                         muse_params = self.set_muse_param(muse_params,layer, sub, bin_name)
    #                         task = pool.apply_async(process_task, args=(layer, sub, bin_name, extend_exp, self.model_name, self.do, muse_params))
    #                         tasks.append(task)
    #                         metrics_df = pd.concat([metrics_df, pd.DataFrame(task.get(), index=[0])])
    #
    #
...
def muse_supervised(configs):
    ...
    src_normal_emb = normalization(torch.load(params.src_emb)["vectors"])
    src_emb.weight.data.copy_(src_normal_emb)
    logger.info("Manually Normalization for Target")
    tgt_normal_emb = normalization(torch.load(params.tgt_emb)["vectors"])
