...
class RidgeRegression(FMRIWordLevel):
    def __init__(self, config):
        ...
        self.layers = config.models.n_layers
    ...
    def run_regression(self, fmri_type="token"):
        (
            regression_files_exist,
            preds_save_root,
            word_embs_save_path,
        ) = self.setup_word_emb_and_regression_path(fmri_type)
        ...
        for layer in range(self.layers):
            for sub in range(1, self.subjects + 1):
                all_preds = []
                all_words = []
                lm_data, brain_data, words, ind, corrs_lm = self.load_lm_brain_data(
                    word_embs_save_path, layer, sub, words, fmri_type
                )
                for ind_num in range(self.num_folds):
                    ...
                    # Do regression
                    start = time.time()
                    weights, chosen_lambdas = ridge_tools_torch.cross_val_ridge(
                        brain_train_data,
                        lm_inflate_train_label,
                        n_splits=10,
                        lambdas=np.array(
                            [10**i for i in range(-6, 10)], dtype="float32"
                        ),
                    ...
                    corrs_lm[ind_num, :] = ridge_tools_torch.corr_torch(
                        preds, lm_inflate_test_label
                    )
    ...
    def setup_word_emb_and_regression_path(self, fmri_type):
        preds_save_root = (
            Path(f"{self.word_emb_unique_save_dir}_regression-{fmri_type}")
            / self.model_name
        )
        regression_files_exist = all(
            (
                (
                    preds_save_root
                    / f"{self.dataset_name}_{self.model_name}_dim_{self.lm_dim_size}_layer_{layer}_sub_{sub}.pth"
                ).exists()
                for layer in range(self.layers)
                for sub in range(1, self.subjects + 1)
            )
        ...
        if self.model_name == "fasttext" or self.model_alias == "openai":
            word_embs_save_path = Path(
                f"{self.word_emb_unique_save_dir}/{self.suffix}/{self.model_name}"
            )
        else:
            word_embs_save_path = Path(
                f"{self.word_emb_unique_save_dir}_{self.suffix}/{self.model_name}"
            )
    ...
    def load_lm_brain_data(self, word_embs_save_path, layer, sub, words, fmri_type):
        ...
        brain_data = torch.load(
            self.outfile_dir
            / f"{self.dataset_name}-{fmri_type}-sub--{sub}-{self.lookback}-{self.lookout}-0.pth"
        )
        ...
        if self.model_name == "fasttext" or self.model_alias == "openai":
            lm_file_path = (
                word_embs_save_path
                / f"{self.dataset_name}_{self.model_name}_dim_{self.lm_dim_size}_layer_0.pth"
            )
            lm_data = torch.load(lm_file_path)
        else:
            lm_file_path = (
                word_embs_save_path
                / f"{self.dataset_name}_decon_{self.model_name}_dim_{self.lm_dim_size}_layer_{layer}.pth"
            )
            lm_data = torch.load(lm_file_path)
    ...
    def get_lm_inflate_label(lm_data, words, brain_words, if_cased):
        ...
        vecs = [
            lm_data["vectors"][lm_data["dico"].index(word)]
            for dico_word in dico_words
            for word in words
            if word == dico_word
        ]
        # vecs = np.vstack(vecs)
    ...
    def prepare_train_test_data(self, lm_data, brain_data, words, ind, ind_num):
        ...
        def get_split_string(if_cased):
            return (
                lambda word: word.split("_")[0]
                if if_cased == "cased"
                else word.lower().split("_")[0]
            )

        split_string_fn = get_split_string(self.if_cased)
        brain_train_data_indx = np.where(
            np.isin([split_string_fn(word) for word in brain_data["dico"]], train_words)
        )[0]
        brain_test_data_indx = np.where(
            np.isin([split_string_fn(word) for word in brain_data["dico"]], test_words)
        )[0]
        ...
        lm_inflate_train_label = self.get_lm_inflate_label(
            lm_data, train_words, brain_data["dico"], self.if_cased
        )
        lm_inflate_test_label = self.get_lm_inflate_label(
            lm_data, test_words, brain_data["dico"], self.if_cased
        )
        return (
            brain_train_data,
            brain_test_data,
    ...
    def save_predictions(
        self, preds_save_root, layer, sub, all_words, all_preds, corrs_lm
    ):
        ...
        torch.save(
            {"dico": all_words, "vectors": all_preds, "corr": corrs_lm},
            str(preds_save_root / fname),
        )
