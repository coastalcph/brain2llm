...
class Muse(FMRIWordLevel):
    def __init__(self, config, train_eval, fmri_type="token"):
        ...
        self.do = train_eval
        self.vec_dim = (
            min(config.datasets.tr_num, config.models.dim)
            if self.do == "train"
            else config.models.dim
        )
    ...
    def set_muse_param(self, layer, sub, bin_name, fold=None):
        ...
        if (
            self.model_alias == "ft"
            or self.model_alias == "openai"
            or self.model_alias == "bg"
        ):
                ...
                for bin_name in bins:
                    ...
                    if self.do == "train":
                        for fold in range(self.num_folds):
                            ...
                            muse_params = self.set_muse_param(
                                layer, sub, bin_name, fold
                            )
